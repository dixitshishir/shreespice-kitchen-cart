...
const Cart = ({
  ...
  const countryCodes = [{
    ...
    name: 'United States',
    digits: 10,
    gradient: 'linear-gradient(135deg, #3C3B6E 0%, #FFFFFF 50%, #B22234 100%)'
  }, {
    code: '+44',
    country: '🇬🇧 UK',
    name: 'United Kingdom',
    digits: 10,
    gradient: 'linear-gradient(135deg, #012169 0%, #FFFFFF 50%, #C8102E 100%)'
  }, {
    code: '+971',
    country: '🇦🇪 Dubai/UAE',
    name: 'United Arab Emirates',
    digits: 9,
    gradient: 'linear-gradient(135deg, #00732F 0%, #FFFFFF 40%, #000000 70%, #FF0000 100%)'
  }, {
    code: '+49',
    country: '🇩🇪 Germany',
    name: 'Germany',
    digits: 11,
    gradient: 'linear-gradient(135deg, #000000 0%, #DD0000 50%, #FFCC00 100%)'
  }, {
    code: '+61',
    country: '🇦🇺 Australia',
    name: 'Australia',
    digits: 9,
    gradient: 'linear-gradient(135deg, #00008B 0%, #FFFFFF 50%, #FF0000 100%)'
  }, {
    code: '+65',
    country: '🇸🇬 Singapore',
    name: 'Singapore',
    digits: 8,
    gradient: 'linear-gradient(135deg, #ED2939 0%, #FFFFFF 100%)'
  }, {
    code: '+1-CA',
    country: '🇨🇦 Canada',
    name: 'Canada',
  ...
  const handleSubmitOrder = () => {
    ...
    const orderItems = items.map(item => `${item.product.name} - ${item.quantity} unit(s) (${item.quantity * 500}g)`).join('\n');
...
${isDavangere ? '🏠 *Collection:* Collect from Dixit Offset Printers or from our home.' : '📦 *Delivery:* This order will be couriered to you. Courier charges will be calculated based on your location. Usually takes 1-2 days to prepare the order.'}
...
*Nalini Dixit's Shree Spices and Snacks* 📱 ${motherPhone}`;
  ...
  const StepIndicator = () => <div className="flex items-center justify-center gap-2 mb-4">
      <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${step === 'summary' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
        <Package className="h-3.5 w-3.5" />
        <span>Order</span>
      </div>
      <div className="w-6 h-0.5 bg-muted rounded" />
      <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all ${step === 'details' ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>
  ...
  if (step === 'summary') {
    return <Dialog open={isOpen} onOpenChange={resetAndClose}>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gradient-to-br from-secondary/80 to-muted/60 rounded-xl p-4 space-y-3">
              ...
              <div className="space-y-2.5 max-h-[35vh] overflow-y-auto pr-1">
                {items.map(item => <div key={item.product.id} className="flex items-center gap-3 bg-background/80 rounded-lg p-2.5 shadow-sm">
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="outline" onClick={() => setStep('cart')} className="flex-1 rounded-xl border-border hover:bg-secondary h-12">
                <ArrowLeft className="h-4 w-4 mr-2" />
                Edit Cart
              </Button>
              <Button onClick={() => setStep('details')} className="flex-1 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl h-12 font-semibold">
                Next
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input id="name" value={customerDetails.name} onChange={e => setCustomerDetails({
                ...customerDetails,
                name: e.target.value
              })} placeholder="Enter your full name" className="mt-1.5 h-11 rounded-xl border-border focus:ring-2 focus:ring-primary/20 bg-background text-foreground" />
              </div>

              <div>
                <Label htmlFor="phone" className="text-sm font-medium text-foreground">Phone Number *</Label>
                <div className="flex gap-2 mt-1.5">
                  <select value={customerDetails.countryCode} onChange={e => handleCountryCodeChange(e.target.value)} style={{
                  background: getCountryGradient()
                  ...
                  <Input id="phone" type="tel" inputMode="numeric" value={customerDetails.phone} onChange={e => handlePhoneChange(e.target.value)} placeholder={`${getPhoneDigitLimit()} digit number`} maxLength={getPhoneDigitLimit()} className="h-11 rounded-xl border-border flex-1 bg-background text-foreground" />
                </div>
                {customerDetails.phone.length > 0 && customerDetails.phone.length < getPhoneDigitLimit() && <p className="text-xs text-amber-600 mt-1">Enter {getPhoneDigitLimit()} digit phone number</p>}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <Textarea id="address" value={customerDetails.address} onChange={e => setCustomerDetails({
                ...customerDetails,
                address: e.target.value
              })} placeholder="House/Flat No, Street, Area" className="mt-1.5 rounded-xl border-border resize-none focus:ring-2 focus:ring-primary/20 bg-background" rows={2} />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <Label htmlFor="city" className="text-sm font-medium text-foreground">City *</Label>
                  <Input id="city" value={customerDetails.city} onChange={e => setCustomerDetails({
                  ...customerDetails,
                  city: e.target.value
                })} placeholder="City" className="mt-1.5 h-11 rounded-xl border-border bg-background text-foreground" />
                </div>
                <div>
                  <Label htmlFor="pincode" className="text-sm font-medium text-foreground">PIN Code</Label>
                  <Input id="pincode" value={customerDetails.pincode} onChange={e => setCustomerDetails({
                  ...customerDetails,
                  pincode: e.target.value
                })} placeholder="PIN Code" className="mt-1.5 h-11 rounded-xl border-border bg-background text-foreground" />
                </div>
              </div>

              <div>
                <Label htmlFor="landmark" className="text-sm font-medium text-foreground">Landmark (Optional)</Label>
                <Input id="landmark" value={customerDetails.landmark} onChange={e => setCustomerDetails({
                ...customerDetails,
                landmark: e.target.value
              })} placeholder="Nearby landmark" className="mt-1.5 h-11 rounded-xl border-border bg-background text-foreground" />
              </div>
            </div>
            ...
            <div className="bg-secondary/60 rounded-xl p-4 border border-border">
              ...
              <div className="space-y-2 text-xs text-muted-foreground">
                ...
                <p className="flex items-start gap-2">
                  <span className="font-bold text-primary">Other Cities:</span>
            ...
            {/* Action Buttons */}
            <div className="flex gap-3 pt-2">
              <Button variant="outline" onClick={() => setStep('summary')} className="flex-1 rounded-xl border-border hover:bg-secondary h-12">
                <ArrowLeft className="h-4 w-4 mr-2" />
                Back
              </Button>
              <Button onClick={handleSubmitOrder} className="flex-1 bg-green-600 hover:bg-green-700 text-white rounded-xl h-12 font-semibold shadow-lg shadow-green-600/20">
      ...
      <DialogContent className="w-[92vw] max-w-md max-h-[85vh] overflow-y-auto p-5 rounded-2xl border-border bg-card backdrop-blur-md shadow-xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl font-bold text-foreground">
            ...
            Your Cart
            {items.length > 0 && <span className="ml-auto text-sm font-normal text-muted-foreground">
        ...
        <div className="space-y-4">
          {items.length === 0 ? <div className="text-center py-12">
              ...
              <div className="space-y-3 max-h-[45vh] overflow-y-auto pr-1">
                {items.map(item => <div key={item.product.id} className="bg-gradient-to-br from-secondary/60 to-muted/40 rounded-xl p-3 shadow-sm">
                    {/* Top row: Image, Name, Delete */}
                    <div className="flex items-start gap-3 mb-2">
                      <img src={item.product.image} alt={item.product.name} className="w-14 h-14 object-cover rounded-lg flex-shrink-0 shadow-sm" />
                      <div className="flex-1 min-w-0">
                      ...
                      <Button size="sm" variant="ghost" onClick={() => removeFromCart(item.product.id)} className="h-8 w-8 p-0 text-red-500 hover:text-red-600 hover:bg-red-50 flex-shrink-0 rounded-full">
                    ...
                    <div className="flex items-center justify-between bg-background/70 rounded-lg p-2">
                      <div className="flex items-center gap-1">
                        <Button size="sm" variant="outline" onClick={() => updateQuantity(item.product.id, item.quantity - 1)} className="h-8 w-8 p-0 rounded-full bg-white shadow-sm">
              ...
              <Button onClick={handleProceedToSummary} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground py-3 h-12 rounded-xl font-semibold shadow-lg shadow-primary/20">
